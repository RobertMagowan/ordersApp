...
- Sections 25–35 of the source handoff at `C:\Users\admin\Documents\Codex\2026-08-16\referenced-chatgpt-conversation-this-is-an\outputs\CLOUDORDERS_HANDOFF.md` are the authoritative version-1 contracts. Sprint 2 must add a repository-owned contract index so future execution does not depend only on that machine-local path.
...
- [ ] Add a repository-owned version-1 contract pack under `docs/contracts/` that restates the handoff's sections 25–35 and maps each requirement to the implementing projects, tests, and sprint gates, removing runtime dependence on the machine-local source.
