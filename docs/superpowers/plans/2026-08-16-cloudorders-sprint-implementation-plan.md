...
- Each task within a sprint is also a development release gate: it must pass TDD/local verification, independent review, a protected `feature/*` -> `development` PR/deployment, and dedicated Azure development destruction testing before the next task starts. Defects block the next task until fixed/reviewed/redeployed/retested. When all tasks pass individually, a separate full-sprint development destruction pass precedes the single normal `development` -> `test` PR and Azure test QA; task-level development releases never bypass the promotion model.
...
- [x] Reconcile contracts and all later-sprint references to use bare delegated scopes `Orders.Read`/`Orders.Write`, exact role `user.admin`, verified External ID customer, `CustomerProfileId`, actor/target idempotency, and safe absent/foreign `404`; correct migration-job polling to capture and poll its exact started execution. E1 reads only `ops/releases/sprint-4a-e1-migration-only.json` on protected development/test pushes, validates `AddCustomerProfileOwnershipExpand` with `deployApi: false`, and preserves API revision, digest, and traffic.
