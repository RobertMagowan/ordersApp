...
| Sections 26/27/32: verified External ID customer, `/me`, bare delegated scopes, `user.admin`, `CustomerProfileId`, and actor/target idempotency | `src/CloudOrders.Api`, `src/CloudOrders.Infrastructure`, `src/CloudOrders.Contracts` | authorization/profile/idempotency integration tests (Sprint 4A) | Sprint 4A identity and ownership |
| Section 33: same immutable artifact, expand/migrate/contract, E1 migration-only manifest, exact Job execution polling, and rollback | `.github/workflows/deploy.yml`, `infra/`, `ops/releases/`, `ops/runbooks/` | `DeploymentWorkflowPolicyTests.DeploymentWorkflowPollsOnlyTheStartedMigrationExecution`; `DeploymentWorkflowPolicyTests.DeploymentWorkflowSupportsTheExactSprint4AE1MigrationOnlyManifest` | Sprint 4A E1 migration-only release; Sprint 14 release automation |
