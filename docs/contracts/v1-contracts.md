...
The API base is `/api/v1`; JSON is UTF-8 camelCase, timestamps are UTC ISO-8601, and statuses are strings. CI-generated OpenAPI is the client source. Required scopes are `CloudOrders.Orders.Read`, `CloudOrders.Orders.Write`, and non-production `CloudOrders.TestOperate`. The API derives authorized customer scope from claims; a cross-customer lookup returns 404 rather than disclosing existence.
...
Application tables use `dbo`; `testsupport` is non-production only. Orders holds identity, CustomerReference (nvarchar(64)), ProductSku (nvarchar(64)), Quantity (1–100), Status constrained to Pending/Processing, CreatedAt, UpdatedAt, and RowVersion, indexed by `(CustomerReference, CreatedAt DESC, Id DESC)`.

OutboxMessages is keyed by EventId and holds aggregate/order ID, message type/version, payload, occurrence/creation timestamps, processed timestamp, attempts/error code, and W3C trace values. Its filtered pending index is `(CreatedAt, Id)` including message/version/occurred/attempt fields. InboxMessages is keyed by EventId and records Handler, last broker message ID, processed timestamp, and trace ID. IdempotencyRecords has primary key `(SubjectId, IdempotencyKey)` and stores request hash, OrderId, initial response status/JSON, created and expiry timestamps. Order, Outbox, and Idempotency are inserted in a single transaction; duplicate-key races are re-read and classified as replay or conflict.
