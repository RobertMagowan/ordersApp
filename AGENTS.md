...
- Git promotion is `feature/*` → `development` → `test` → `master`; all new feature branches must use the `feature/` prefix. Protected-branch changes require a pull request, required checks, and conversation resolution. Merge every protected-branch PR using GitHub's **Create a merge commit** option: squash and rebase merges are disabled so the merge preserves the source and target lineage. The PR-path check runs from the trusted base branch, while the post-merge check validates the two-parent lineage. Because this is a single-developer repository, zero independent approvals are required; the administrator remains responsible for reviewing and merging.
...
Treat every task as a development release increment: do not start the next task until the current task has passed local verification, independent review, protected `feature/*` -> `development` promotion, Azure development destruction testing, and all required remediation/re-tests. After every task in a sprint passes this gate, perform a separate full-sprint development destruction pass before the single `development` -> `test` PR and Azure test QA. Task-level releases must preserve—not bypass—the branch protection and environment approval workflow.
