...
Sprint 3 adds a non-production Azure SQL gate: development and test build immutable API and migration-runner images, preview and provision Entra-only Azure SQL, then run the one-shot migration Job before promoting the API candidate. The temporary Entra SQL administrator must run [the contained-user bootstrap](docs/operations/azure-sql-bootstrap.md) before the first job execution. Production remains unchanged by this sprint.
