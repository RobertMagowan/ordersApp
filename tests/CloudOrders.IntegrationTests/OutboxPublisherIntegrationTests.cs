...
public sealed class OutboxPublisherIntegrationTests
{
    ...
    {
        var store = new FakeStore(Lease());
        var publisher = new OutboxPublisherFunction(store, new RecordingSender { Cancel = true }, new TestTimeProvider());
    ...
    private sealed class RecordingSender : IOutboxMessageSender
    {
        ...
        { if (Failure is not null) throw Failure; if (Cancel) throw new OperationCanceledException(cancellationToken); MessageId = message.EventId; Payload = message.Payload; return Task.CompletedTask; }
    }
